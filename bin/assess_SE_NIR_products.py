...
"""
Perfoms an assessent of exposures from a first/final cut reduction of NIR data.
The quality derived is based on the seeing (FWHM), background, and extinction
due to clouds (similar to for DECam... note these values, especially their relative
scaling have not been checked and so cannot be assusmed to be equivalent to those
used for DES data.
...
#    parser.add_argument('-A', '--AttNum',   action='store', type=str, default=None, help='Attempt Number.')
    parser.add_argument('-C', '--CamSym',   action='store', type=str, default='V', help='CamSym (default=\"V\").')
    parser.add_argument('-l', '--list',     action='store', type=str, default=None, 
                        help='Optional list of catalogs for the assessment (otherwise query for "cat_finalcut" catalogs associated with the Request Unit Attempt)')
...
#                         help='Flag to dump file containing raw data summarizing number of objects per magnitude bin.')
    ...
    parser.add_argument('-s', '--section', action='store', type=str, default='db-desoper', help='section of .desservices file with connection info')
...
#
#   This option may soon be deprecate-able (RAG 9/2/2022)
    parser.add_argument('--oe', action='store', type=int, default=None, help='Overide exposure number (stop gap to allow discovery of exposure info when assessing VSA data)')
...
#   New veresion (an additive offset needed when comparing FWHM_MEAN (from PSFex) with respect to QC)
    ...
    band2i={'u':0,'g':1,'r':2,'i':3,'z':4,'Y':5,'VR':6,'N964':7,'VZ':8,'VY':9,'J':10,'H':11,'Ks':12}
#
...
#    ellip_good=0.07
#
    kolmogorov={'u':1.2,'g':1.103,'r':1.041,'i':1.00,'z':0.965,'Y':0.95,'VR':1.04,'N964':0.965,'VY':1.0,'J':1.0,'H':1.0,'Ks':1.0}
    teff_lim={  'u':0.2,'g':0.2,  'r':0.3,  'i':0.3, 'z':0.3,  'Y':0.2,'VR':0.3,'N964':0.3,'VY':0.2,'J':0.2,'H':0.2,'Ks':0.2}
    seeing_lim={}
    seeing_fid={}
#
...
#
    for band in ['u','g','r','i','z','Y','VR','N964','VY','J','H','Ks']:
        if (band == "Y"):
...
#   NIR came from Table 4 of https://manualzz.com/doc/o/a5c60/very-large-telescope-paranal-science-operations-vircam-6.3-filters
#
    sbrite_good={"u":0.2,"g":1.05,"r":2.66,"i":7.87,"z":16.51,"Y":14.56,"VR":3.71,"N964":0.4,
                 "VY":64.,"J":254.,"H":1376.,"Ks":1925.}
    sbrite_lim={"u":0.8,"g":4.0,"r":9.58,"i":21.9,"z":50.2,"Y":27.6,"VR":13.58,"N964":20.2,
                 "VY":2000.,"J":2000.,"H":5000.,"Ks":5000.}
...
#
    ...
    cat_mag_corr={'apass':{'u':3.5,'g':0.205,'r':0.128,'i':0.112,'z':0.0,'Y':0.0,'VR':0.0,'N964':3.56,'H':0.0},
                  'nomad':{'u':3.65,'g':0.341,'r':0.235,'i':1.398,'z':1.201,'Y':1.083,'VR':0.235,'N964':4.67,'H':0.0},
                  'des':{'u':0.0,'g':0.248,'r':0.175,'i':0.078,'z':0.08,'Y':0.06,'VR':0.0,'N964':3.56,'VY':0.0},
                  '2mass':{'VZ':0.0,'VY':0.0,'J':0.0,'H':0.0,'Ks':0.0},
                  'vista':{'VZ':0.0,'VY':0.0,'J':0.0,'H':0.0,'Ks':0.0}}
    cat_kmag_corr={'apass':{'u':0.0,'g':0.000,'r':0.000,'i':0.000,'z':0.0,'Y':0.0,'VR':0.0,'N964':0.0,'H':0.0},
                  'nomad':{'u':0.0,'g':0.111,'r':0.109,'i':1.289,'z':1.139,'Y':1.022,'VR':0.0,'N964':0.0,'H':0.0},
                  'des':{'u':0.0,'g':0.000,'r':0.000,'i':0.000,'z':0.0,'Y':0.0,'VR':0.0,'N964':0.0,'VY':0.0},
                  '2mass':{'VZ':0.0,'VY':0.0,'J':0.0,'H':0.0,'Ks':0.0},
                  'vista':{'VZ':0.0,'VY':0.0,'J':0.0,'H':0.0,'Ks':0.0}}
#
...
#
#    aterm=numpy.zeros(13,dtype=numpy.float32)
    aterm={
        'u':-25.0,'g':-25.428,'r':-25.532,'i':-25.413,'z':-25.086,'Y':-24.000,'VR':-25.47,'N964':-25.086,
        'VZ':0.502,'VY':0.600,'J':0.916,'H':1.366,'Ks':1.827,'NB118':0.853
    }

#
#   A set of kterm(s) that represent an estimate for the nominal extinction/airmass correction on a clear, photometric night.
#
#    kterm=numpy.zeros(9,dtype=numpy.float32)
    kterm={'u':0.489,'g':0.181,'r':0.095,'i':0.089,'z':0.053,'Y':0.05,'VR':0.13,'N964':0.053,
        'VZ':0.053,'VY':0.05,'J':0.02,'H':0.02,'Ks':0.02,'NB118':0.02}


#   https://manualzz.com/doc/o/a5c60/very-large-telescope-paranal-science-operations-vircam-6.3-filters (table 4)
#   Has potential kterm, mag_zero  1 ADU/sec --> 23.82,23.45,23.78,23.87,23.03
...
#           The case where gains are not 1... therefore units are already in counts
        ...
        if (ccd_info[ccdnum]['mag_zero'] is None):
            ccd_info[ccdnum]['mag_zero']=25.
...
#
    ...
    exp_rec['mag_zero']=numpy.mean(numpy.array([ccd_info[k]['mag_zero'] for k in ccd_info]))
...
#
    ...
    if (args.oe is not None):
        exp_rec['expnum']=args.oe

    query = """
        select e.filename as exp_fname,
            e.expnum,
            e.nite,
            'date_obs' as date_obs, 'time_obs' as time_obs, e.mjd_obs,
            e.obstype, 
            e.band,
            e.camsym,
            'object' as object,
            'telra' as telra, 'teldec' as teldec,
            e.ra as tradeg, e.dec as tdecdeg,
            e.airmass,
            e.exptime,
            'program' as program,
            'field' as field
        from {schema:s}nir_paw e
        where e.expnum={expnum:}
            and e.filetype='nir_paw'
...
#
    ...
    if (exp_rec['band'] in ['VZ','VY','J','H','Ks','NB118']):
        qparse['2mass']={'pname':'2MASS','tab':'TWOMASS_PSC','tab_abbrev':'m','db':'sci','ra':'m.ra','dec':'m.dec'}
        if (exp_rec['band'] in ['VZ','VY','J','NB118']):
            qparse['2mass']['mag']='m.j+0.916'
            qparse['2mass']['dmag']='m.j_cmsig'
            qparse['2mass']['mlimit']=jlimit+0.916
        elif (exp_rec['band'] in ['H']):
            qparse['2mass']['mag']='m.h+1.366'
            qparse['2mass']['dmag']='m.h_cmsig'
            qparse['2mass']['mlimit']=hlimit+1.366
        elif (exp_rec['band'] in ['Ks']):
            qparse['2mass']['mag']='m.k+1.827'
            qparse['2mass']['dmag']='m.k_cmsig'
            qparse['2mass']['mlimit']=klimit+1.827
        qparse['2mass']['simple']=True
        qparse['2mass']['keys']=['ra','dec','mag','dmag']
    else:
        print("Band={:s} not supported for 2MASS calibration.  Will skip".format(exp_rec['band']))

    if (exp_rec['band'] in ['VZ','VY','J','H','Ks','NB118']):
        qparse['vista']={'pname':'VHS','tab':'des_admin.VHS_DR5','tab_abbrev':'v','db':'sci','ra':'v.ra2000','dec':'v.dec2000'}
        if (exp_rec['band'] in ['VZ','VY']):
            qparse['vista']['mag']='v.yapermag3+0.600'
            qparse['vista']['dmag']='v.yapermag3err'
            qparse['vista']['mlimit']=jlimit+0.600
        elif (exp_rec['band'] in ['J','NB118']):
            qparse['vista']['mag']='v.japermag3+0.916'
            qparse['vista']['dmag']='v.japermag3err'
            qparse['vista']['mlimit']=jlimit+0.916
        elif (exp_rec['band'] in ['H']):
            qparse['vista']['mag']='v.hapermag3+1.366'
            qparse['vista']['dmag']='v.hapermag3err'
            qparse['vista']['mlimit']=hlimit+1.366
        elif (exp_rec['band'] in ['Ks']):
            qparse['vista']['mag']='v.ksapermag3+1.827'
            qparse['vista']['dmag']='v.ksapermag3err'
            qparse['vista']['mlimit']=klimit+1.827
        qparse['vista']['simple']=True
        qparse['vista']['keys']=['ra','dec','mag','dmag']
    else:
        print("Band={:s} not supported for VHS_DR5 calibration.  Will skip".format(exp_rec['band']))
    ...
    elif (exp_rec['band'] in ['VY']):
        qparse['des']={'pname':'Y3A2','tab':'Y3A2_COS_SUBSET','tab_abbrev':'cos','db':'oper','ra':'cos.alphawin_j2000','dec':'cos.deltawin_j2000'}
        qparse['des']['mag']='cos.wavg_mag_psf_{:s}'.format('Y')
        qparse['des']['dmag']="cos.wavg_magerr_psf_{:s}".format('Y')
        qparse['des']['spread_model']="cos.wavg_spread_model_{:s}".format('Y')
        qparse['des']['spreaderr_model']="cos.wavg_spreaderr_model_{:s}".format('Y')
        qparse['des']['mlimit']=glimit
        qparse['des']['keys']=['ra','dec','mag','dmag','spread_model','spreaderr_model']
...
#
    ...
    if ('2mass' in qparse):
        qparse['2mass']['constraint']=' and {m1:s}<{ml1:.1f}'.format(
            m1=qparse['2mass']['mag'],
            ml1=qparse['2mass']['mlimit'])
    if ('vista' in qparse):
        qparse['vista']['constraint']=' and {m1:s}<{ml1:.1f} and v.primary_source=1'.format(
            m1=qparse['vista']['mag'],
            ml1=qparse['vista']['mlimit'])
    if ('des' in qparse):
...
#
        ...
        for cat in qparse:
            qparse[cat]['query']="""
                ...
                where {dcol:s} between {d1:.7f} and {d2:.7f} 
                   and ({rcol:s} < {r1:.7f} or {rcol:s} > {r2:.7f})
                    {mcon:s}""".format(
                qlist=qparse[cat]['qselect'],
                tname=qparse[cat]['tab'],tabbrev=qparse[cat]['tab_abbrev'],
                dcol=qparse[cat]['dec'],rcol=qparse[cat]['ra'],
...
#
        ...
        for cat in qparse:
            qparse[cat]['query']="""
                select {qlist:s} from {tname:s} {tabbrev:s}
                where {dcol:s} between {d1:.7f} and {d2:.7f} 
                    and {rcol:s} between {r1:.7f} and {r2:.7f}
                    {mcon:s}""".format(
                qlist=qparse[cat]['qselect'],
                tname=qparse[cat]['tab'],tabbrev=qparse[cat]['tab_abbrev'],
                dcol=qparse[cat]['dec'],rcol=qparse[cat]['ra'],
...
##    print("RAG: bval=",bval)
#
    if (exp_rec['band'] in ['u','g','r','i','z','Y','VR','N964']):
        ...
        if (iccd in ccd_info):
            if (os.path.isfile(ccd_info[iccd]['cat_fpath'])):
                ...
                if ('ALPHAWIN_J2000' in exp_cat):
                    ...
                    tmp_magzero=numpy.zeros(CAT['ALPHAWIN_J2000'].size,dtype=numpy.float32)
                    tmp_magzero+=ccd_info[iccd]['mag_zero']
                    newcol=numpy.concatenate((exp_cat['MAG_ZERO'],tmp_magzero),axis=0)
                    exp_cat['MAG_ZERO']=newcol
                else:
                    for col in cols_retrieve:
                        exp_cat[col]=CAT[col]
                    exp_cat['CCDNUM']=numpy.zeros(CAT['ALPHAWIN_J2000'].size,dtype=numpy.int16)
                    exp_cat['CCDNUM']+=iccd
                    exp_cat['MAG_ZERO']=numpy.zeros(CAT['ALPHAWIN_J2000'].size,dtype=numpy.float32)
                    exp_cat['MAG_ZERO']+=ccd_info[iccd]['mag_zero']
#
...
#    new_hj2xpcat=sorted(exp_cat,key=lambda k: k['MAG_'+use_mag_type])
...
#       Below (use of mtime) attempted to compensate for ESO/VSA zeropoint conventions (this is now fixed in nir_rehab).
#        mag=exp_cat['MAG_'+use_mag_type]+exp_cat['MAG_ZERO']+mcorr+mtime
        mag=exp_cat['MAG_'+use_mag_type]+exp_cat['MAG_ZERO']+mcorr
...
#             "nomad":['u','g','r','i','z','Y','VR']}
    ...
    if ('des' in cat_match):
        cat_match['des']['banduse']=['g','r','i','z','Y','N964','VY']
    if ('2mass' in cat_match):
        cat_match['2mass']['banduse']=['VZ','VY','J','H','Ks']
    if ('vista' in cat_match):
        cat_match['vista']['banduse']=['VZ','VY','J','H','Ks']
...
#           Below (use of mtime) attempted to compensate for ESO/VSA zeropoint conventions (this is now fixed in nir_rehab).
#            expCat['MAG']=exp_cat['MAG_'+use_mag_type]+exp_cat['MAG_ZERO']+mcorr+mtime
            expCat['MAG']=exp_cat['MAG_'+use_mag_type]+exp_cat['MAG_ZERO']+mcorr
...
#
    ...
    for cat in ['des','apass','nomad','2mass','vista']:
        t400=time.time()
        print("########################################")
        doNewComp=True
...
#
                ...
                exp_rec[cat+"_kmagdiff"]=med_magdiff-cat_mag_corr[cat][exp_rec['band']]+cat_kmag_corr[cat][exp_rec['band']]-(kterm[exp_rec['band']]*exp_rec['airmass'])
...
#
#   Calculate C_eff
#   (use difference between MAG_ZERO and fiducial values (if available))
#   Fiducials from Table 4  of https://manualzz.com/doc/o/a5c60/very-large-telescope-paranal-science-operations-vircam-6.3-filters
#   Conversion to Vega2AB
#
    fm0={'VZ':23.82,
        'VY':23.45,
        'J':23.78,
    ...
    if (('mag_zero' in exp_rec)and(exp_rec['band'] in fm0)):
        zpt_diff=fm0[exp_rec['band']] + (2.5*numpy.log10(exp_rec['exptime'])) + vega2ab[exp_rec['band']] - exp_rec['mag_zero']
        print(" C_EFF calculated from difference between zeropoint and a fiducial")
        exp_rec['cloud_cat']='ZP_FDIFF'
        if (zpt_diff<0.2):
            exp_rec['teff_c']=1.0
        else:
            exp_rec['teff_c']=math.pow(10.0,(-2.0*(zpt_diff-0.2)/2.5))

    elif ((exp_rec['magdiff']>-95.)and(exp_rec['magdiff']<95.0)):
...
#   Summary of all FWHM measurements
    ...
    if ('vista' in cat_match): print("#       query VISTA: {:7.2f} ".format(cat_match['vista']['timing']))
    if ('2mass' in cat_match): print("#       query 2MASS: {:7.2f} ".format(cat_match['2mass']['timing']))
    ...
    if ('vista' in cat_match): print("#  cross-corr VISTA: {:7.2f} ".format(cat_match['vista']['crosstiming']))
    if ('2mass' in cat_match): print("#  cross-corr 2MASS: {:7.2f} ".format(cat_match['2mass']['crosstiming']))
...
#
    if ((exp_rec['2mass_magdiff']>-95.)and(exp_rec['2mass_magdiff']<95.)):
        db_cname=db_cname+','+'CLOUD_2MASS'
        db_value=db_value+','+"%.3f" % exp_rec['2mass_magdiff']

    if ((exp_rec['vista_magdiff']>-95.)and(exp_rec['vista_magdiff']<95.)):
        db_cname=db_cname+','+'CLOUD_VISTA'
        db_value=db_value+','+"%.3f" % exp_rec['vista_magdiff']
    ...
    if ((exp_rec['2mass_kmagdiff']>-95.)and(exp_rec['2mass_kmagdiff']<95.)):
        db_cname=db_cname+','+'KLOUD_2MASS'
        db_value=db_value+','+"%.3f" % exp_rec['2mass_kmagdiff']

    if ((exp_rec['vista_kmagdiff']>-95.)and(exp_rec['vista_kmagdiff']<95.)):
        db_cname=db_cname+','+'KLOUD_VISTA'
        db_value=db_value+','+"%.3f" % exp_rec['vista_kmagdiff']
    ...
    if (exp_rec['2mass_num']>-1):
        db_cname=db_cname+','+'N_2MASS'
        db_value=db_value+','+"%d" % exp_rec['2mass_num']

    if (exp_rec['vista_num']>-1):
        db_cname=db_cname+','+'N_VISTA'
...
#
    ...
    ftxt.write("# Exposure                                     FWHM                             #        AIR    EXP  SCMP  HighSN     Depth       2MASS           VISTA            DES       FWHM            \n")
...
#
    ftxt.write(" {expnum:9d} {decide:4s}  {teff:6.2f} {feff:6.2f} {beff:6.2f} {ceff:6.2f}   {fwhm:5.2f} {ellip:6.3f} {skyb:8.2f} {numobj:7d}   {numccd:3d} {band:1s} {iband:1d} {airmass:6.3f} {exptime:6.1f} {dummy:2d} {asig1:6.3f} {asig2:6.3f}  {mdiff:8.3f} {amdiff:8.3f} {anum:6d} {nmdiff:8.3f} {nnum:6d} {dmdiff:8.3f} {dnum:6d} {fwhm_wld:6.3f} {object:s} \n".format(
        ...
        amdiff=exp_rec['2mass_magdiff'],
        anum=exp_rec['2mass_num'],
        nmdiff=exp_rec['vista_magdiff'],
        nnum=exp_rec['vista_num'],
