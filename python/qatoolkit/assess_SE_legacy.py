...
def binsrch_crosscorr_cat(ra, dec, catalog, match_rad):
    """
    Search through a catalog for match to a set of coordinates
     (only nearest match is returned).

    ra        = RA used for comparison
    dec       = Dec used for comparison
    catalog   = is a list containing records(dictionaries) that have an "ra"
                and "dec" entries along with associated information.
                NOTE: This routine assumes that the catalog is RA-ordered so
                that a binary search can be used to find the small range of
...
#
    ...
    if (ra_range1 > ra_range2):
        # print("Need two ranges for search around 0h-24h branch ",
        #       ra1_deg,dec1_deg)
    ...
    for ra_range in ra_range_list:
        # Initiate a binary search to find the appropriate entries in the
        # catalog
        klo = 0
        khi = num_rec - 1
        while (khi - klo > 1):
...
#       print(klo,khi,catalog[klo]["ra"])
        ...
        while ((klo < num_rec-1) and (catalog[klo]["ra"] < ra_range[1])):
            ...
            klo = klo+1
        if (ncheck > 1000):
            # ROBERT --- please check this expression
            print("Warning possible brute force use detected (RA,DEC,ncheck,range): {:.7f},{:.7f},{:d},{:s}".format(ra1_deg,dec1_deg,ncheck,list(map(str,ra_range))))
#       if (ncheck == 0):
